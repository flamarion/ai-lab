...
async def delete_document(document_id: str, user_id: str | None = Query(None)):
    # Check ownership in Postgres FIRST — only delete vectors if the caller
    # actually owns (or has access to) the document.  Without this ordering,
    # a user could wipe another user's private vectors before the DB check
    # returns 404.
    if db.is_available():
