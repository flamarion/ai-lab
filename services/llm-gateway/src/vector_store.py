...
def search(
    ...
    from qdrant_client.models import Filter, FieldCondition, IsNullCondition, MatchValue, PayloadField

    # Build a filter: shared docs OR user's own docs OR pre-migration docs.
    # Pre-existing chunks (before migration 010) have no is_private field,
    # so we include an IsNullCondition to keep them visible.
    query_filter = None
    if user_id:
        query_filter = Filter(
            should=[
