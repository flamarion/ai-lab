...
# ============================================================
                ...
                for srv in servers:
                    ...
                    with col2:
                        if st.button("Remove", key=f"mcp_rm_{srv['name']}", use_container_width=True):
                            try:
                                httpx.delete(f"{GATEWAY_URL}/mcp/servers/{srv['name']}", timeout=10.0)
                                st.rerun()
                            except Exception as e:
                                st.error(str(e))
            else:
            ...
            if mcp_name.strip() and mcp_command.strip():
                ...
                try:
                    resp = httpx.post(
                        ...
                        json={"name": mcp_name.strip(), "command": mcp_command.strip(), "args": args_list},
                        timeout=30.0,
                    )
                    if resp.status_code == 200:
    ...
    if st.button("Restart MCP connections", use_container_width=True):
        try:
            resp = httpx.post(f"{GATEWAY_URL}/mcp/restart", timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
